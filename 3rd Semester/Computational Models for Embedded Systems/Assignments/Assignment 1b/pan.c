...
iniglobals(void)
{
		...
		now.secondsCount = 0;
		now.signal = addqueue(1, 1);
#ifdef VAR_RANGES
		logval("secondsCount", now.secondsCount);
#endif
...
{	int i = 0;
	...
	{	if (t->ty[i] == TIMEOUT_F) return 1;
		...
		switch (t->qu[i]) {
		case 0: break;
		case 1: if (	(t->ty[i] == Q_FULL_F && ( q_full((int) ( now.signal) )))
			 || 	(t->ty[i] == Q_FULL_T && (!q_full((int) ( now.signal) )))
			 || 	(t->ty[i] == Q_EMPT_F && ( !q_len((int) ( now.signal) )))
			 || 	(t->ty[i] == Q_EMPT_T && (  q_len((int) ( now.signal) )))
			    ) return 0; break;
...
{	/* int i; */
	printf("global vars:\n");
	printf("	chan signal (=%d):	len %d:\t", now.signal, q_len(now.signal));
	c_chandump(now.signal);
	printf("	byte   secondsCount:	%d\n", now.secondsCount);
}
void
...
{	/* int i; */
	switch(tp) {
	case 3:
		printf("local vars proc %d (SMC):\n", pid);
		break;
	case 2:
		printf("local vars proc %d (MP):\n", pid);
		break;
	case 1:
		printf("local vars proc %d (SC):\n", pid);
		break;
	case 0:
		printf("local vars proc %d (MS):\n", pid);
		break;
	}
}
void
...
{
	switch (x) {
	case 1: Printf("notifyUser"); break;
	case 2: Printf("notifyCamera"); break;
	case 3: Printf("registerMovement"); break;
	default: Printf("%d", x);
	}
}
void
